...
class Game {
  constructor(ctx, canvasEl, ball) {
    ...
    this.ball = new __WEBPACK_IMPORTED_MODULE_0__ball_js__["a" /* default */]({game: this});
      ...
      for (let r=0; r < this.brickRowCount; r++) {
        ...
        if (brick.status === 1) {
          ...
          && ball.pos[1] < brick.y + this.brickHeight ) {
            if (brick.hasPowerup) {
              this.game.powerups.push(new __WEBPACK_IMPORTED_MODULE_0__powerup__["a" /* default */]({game: this.game}));
            }
            ball.vel[1] *= -1;
            brick.status = 0;
            this.game.score++;
